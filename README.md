...
├── building.html           # Standalone "in development" holding page — NOT part of the
│                           #   main site (no nav bar, noindex, excluded from sitemap.xml).
│                           #   Links out to /portfolio and / (home) only.
...
├── tools/
│   └── generate-og-images.js # Regenerates assets/og/*.png (see "Open Graph images" below)
│
├── assets/
│   ├── og/                 # Per-page Open Graph / Twitter share images (1200x630),
│   │                       #   generated — don't hand-edit, see "Open Graph images" below
│   ├── css/
...
│   │   ├── site-config.js          # ← Centralized config (Google Apps Script endpoint URL)
...
```jsonc
{
    ...
    "name": "Client Name",
    "role": "Title",
    "company": "Company Name",   // rendered as "Title, Company Name"
...
The testimonial submission form (`testimonials.html`) collects name, company, role, email, an optional logo URL, a star rating, and the quote — all except the logo URL are required. Submissions land in the Google Sheet's `Testimonials` tab (see `google-apps-script/SETUP.md`); publish-consented ones get copied into this array by hand, using the submitted logo URL as `avatar` if one was given.
...
The "Start a Project" enquiry form (`index.html`), the contact form (`contact.html`), and the testimonial form (`testimonials.html`) all post to the same Google Apps Script backend, which logs each to its own Google Sheet tab and sends a branded email notification to the admin.

**Setup:** see `google-apps-script/SETUP.md` for step-by-step deployment instructions. The endpoint URL is centralized in **`assets/js/site-config.js`** — update it there once and all three forms pick it up (a form can still override it locally via its own `data-endpoint="..."` attribute if ever needed).

---

## Open Graph images

Each indexable page has its own 1200×630 Open Graph / Twitter share image under `assets/og/`, built from the real site background gradient plus the full logo and logo mark — not hand-designed, generated by `tools/generate-og-images.js` (Playwright renders an HTML template to PNG per page). Don't hand-edit the PNGs.
...
This is dev-only tooling (`package.json` at the repo root) — it doesn't ship to the browser and isn't a build step for deployment; the output PNGs under `assets/og/` are committed like any other static asset.
